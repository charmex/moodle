...
//    print_object($USER);
    ...
    if (count($courses)) {
        ...
        $cids = array();
        //First we get the list of courses to check completion
        foreach ($courses as $c) {
            $cids[] = $c->id;
        }
        $ids = implode(",", $cids);
        //Then we get the status of each course
        $qry = "SELECT cc.course,timecompleted
        FROM mdl_course_completions cc 
        WHERE cc.course IN(" . $ids . ")
        AND userid = " . $USER->id . ";";

        $completiondata = $DB->get_records_sql($qry);

//    print_object($completiondata);

        foreach ($completiondata as $cd) {
            if ($cd->timecompleted > 0) {
                $courses[$cd->course]->completed = 1;
            }
        }
        //print_object($courses);
    }
