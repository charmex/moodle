...
class block_course_overview_renderer extends plugin_renderer_base {
    ...
    public function course_overview($courses, $overviews) {
        ...
        $catss = coursecat::make_categories_list();
        
        /*
         * Adding categories to frontpage course listing
         */
        $listofcats = array();
        ...
        foreach ($listsofcats as $miau) {
            $html .= $this->output->box_start('catbox', "cat-{$miau->id}");
            $html .= html_writer::start_tag('div', array('class' => 'course_title'));
            $html .= $this->output->heading($catss[$miau], 3, 'title');
            
            foreach ($courses as $key => $course) {
                if ($course->category != $miau) {
                    continue;
                }
                ...
                if ($course->id > 0) {
                    ...
                    if ($course->completed == 1) {
                        $html .= "<label style=\"color:blue\">Completo</label>";
                    } elseif (!property_exists($course, 'completed')) {
                        $html .= "<label style=\"color:red\">Pendiente</label>";
                    }
                } else {
