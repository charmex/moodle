...
abstract class grade_export {
    ...
    public $userlist; //filter by user
    ...
    public function grade_export($courses, $groupid = 0, $itemlist = '', $export_feedback = false, $updatedgradesonly = false, $displaytype = GRADE_DISPLAY_TYPE_REAL, $decimalpoints = 2, $onlyactive = false, $usercustomfields = false, $userlist = '') {
        if (is_array($courses)) {
    ...
    function process_form($formdata) {
        ...
        if (isset($formdata->userlist)) {
            $this->userlist = $formdata->userlist;
        }
    }
