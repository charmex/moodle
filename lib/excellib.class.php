...
class MoodleExcelWorksheet {
    ...
    /*
     * Write one strnig somewhere in the worksheet, but with an aligment of 90°
     * 
     */

    public function write_custom_string($row, $col, $str, $format = null) {
        $this->worksheet->getStyleByColumnAndRow($col, $row + 1)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_TEXT);
//        $this->worksheet->getStyleByColumnAndRow($col, $row + 1)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_JUSTIFY);
        $this->worksheet->getStyleByColumnAndRow($col, $row+1)->getAlignment()->setTextRotation(90);
        $this->worksheet->setCellValueExplicitByColumnAndRow($col, $row + 1, $str, PHPExcel_Cell_DataType::TYPE_STRING);
        $this->apply_format($row, $col, $format);
    }
    ...
    public function write_date($row, $col, $date, $format = null) {
        ...
        $exceldate = PHPExcel_Shared_Date::FormattedPHPToExcel(
                        $getdate['year'], $getdate['mon'], $getdate['mday'], $getdate['hours'], $getdate['minutes'], $getdate['seconds']
        );
        ...
        $this->worksheet->getStyleByColumnAndRow($col, $row + 1)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_DATE_DMYSLASH);
        $this->apply_format($row, $col, $format);
    }
