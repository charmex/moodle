...
class auth_plugin_db extends auth_plugin_base {
    ...
    function __construct() {
        ...
        if (empty($this->config->extencoding)) {
			...
			$this->config->extencoding = 'latin1';
        }
    }
    ...
    function user_login($username, $password) {
		...
		$extusername = textlib::convert($username, 'latin1', $this->config->extencoding);
        $extpassword = textlib::convert($password, 'latin1', $this->config->extencoding);
