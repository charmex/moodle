...
$userlist = optional_param('userlist', '', PARAM_ALPHA);
...
//    $export->display_preview();
    ...
    if (count($selectedcourseids) > 0) {
        ...
        print_error(get_string('notenoughparameters', 'report_globalgrades'), 'index.php');
    }
    ...
    foreach ($users as $user) {
        $userlists[$user->id] = $user->username;
    }

    echo html_writer::start_tag('p');
    echo html_writer::checkbox('userfilter', 'true', false, 'Filtrar por usuario');
    echo html_writer::end_tag('p');

    echo html_writer::tag('p', get_string('userlist', 'report_globalgrades') . ' ' .
            html_writer::select($userlists, 'userlist', ''));
    ...
    echo html_writer::start_tag('p');
    echo html_writer::empty_tag('input', array('type' => 'submit', 'value' => get_string('generatereport', 'report_globalgrades'), 'name' => 'generatereport'));
