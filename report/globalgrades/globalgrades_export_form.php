...
//                        $mform->addElement('advcheckbox', 'itemids[' . $grade_item->id . ']', $grade_item->get_name(), null, array('group' => 1));
                        $mform->addElement('advcheckbox', 'itemids[' . $grade_item->id . ']', $courseN->fullname, null, array('group' => 1));
...
//                    
        ...
        if (isset($features['userlist'])) {
            $mform->addElement('select', 'userlist', 'Usuario a filtrar', array($features['userlist'] => $features['userlist']));
            $mform->setType('display', PARAM_INT);
            $mform->setDefault('display', $features['userlist']);
        }
